...
fn process_create_associated_token_account(
        ...
        && associated_token_account_info.owner == spl_token_program_id
    {
        ...
        if let Ok(associated_token_account) = spl_token::state::Account::unpack(&ata_data) {
            if associated_token_account.owner != *wallet_account_info.key {
                let error = AssociatedTokenAccountError::InvalidOwner;
                msg!("{}", error);
                return Err(error.into());
            }
            if associated_token_account.mint != *spl_token_mint_info.key {
...
pub fn process_recover_nested(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    let (amount, decimals) = {
        ...
        let owner_account = spl_token::state::Account::unpack(&owner_account_data)?;
        if owner_account.owner != *wallet_account_info.key {
        ...
        let nested_account = spl_token::state::Account::unpack(&nested_account_data)?;
        if nested_account.owner != *owner_associated_token_account_info.key {
            msg!("Nested associated token account not owned by provided associated token account");
            return Err(AssociatedTokenAccountError::InvalidOwner.into());
        }
        let amount = nested_account.amount;
        ...
        let nested_mint = spl_token::state::Mint::unpack(&nested_mint_data)?;
        let decimals = nested_mint.decimals;
        (amount, decimals)
    };
